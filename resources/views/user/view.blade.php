...
@section('content')
    ...
    <h2 class="sub-header">@lang('characters.title')</h2>
    <div class="row placeholders">
        ...
        @else
            <div class="col-xs-12">
                <i>@lang('user.noCharacter')</i>
            </div>
        @endif
    </div>

    <h2 class="sub-header">@lang('teams.title')</h2>
    <div class="row placeholders">
        @if (count($user->team))
            @foreach ($user->team as $team)
                <div class="col-xs-6 col-sm-3 placeholder">
                    <h4>{{ $team->name }}</h4>
                    <span class="text-muted"><a href="/teams/{{ $team->id }}">@lang('navigation.seeMore')</a></span>
                </div>
            @endforeach
        @else
            <div class="col-xs-12">
                <i>@lang('user.noTeam')</i>
            </div>
        @endif
    </div>
